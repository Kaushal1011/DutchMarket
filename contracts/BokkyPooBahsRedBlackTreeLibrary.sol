...
// Extension of the below library with below and above methods
...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    // returns the node just below current key
    function below(Tree storage self, uint key) private view returns (uint) {
    ...
    // returns the node just above current key
    function above(Tree storage self, uint key) private view returns (uint) {
        uint cursor = self.root;
        uint _above = EMPTY;
        while (cursor != EMPTY) {
            if (key < cursor) {
    ...
    // return smallest key below or equal to key
    function floor(Tree storage self, uint key) public view returns (uint) {
        uint cursor = self.root;
        uint _floor = EMPTY;
        while (cursor != EMPTY) {
            if (key == cursor) {
